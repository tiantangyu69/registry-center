...
import cc.lee.registry.exception.SkipFailbackWrapperException;
import cc.lee.registry.util.ConcurrentHashSet;
import cc.lee.registry.util.NamedThreadFactory;

public abstract class FailbackRegistry extends AbstractRegistry {
	// 定时任务执行器
	private final ScheduledExecutorService retryExecutor = Executors.newScheduledThreadPool(1, new NamedThreadFactory("RegistryCenterFailedRetryTimer", true));
	...
	public FailbackRegistry(URL url) {
        ...
        this.retryFuture = retryExecutor.scheduleWithFixedDelay(new Runnable() {// 执行定时重试任务
            public void run() {
                // 检测并连接注册中心
                try {
                    retry();
    ...
    public void subscribe(URL url, NotifyListener listener) {
        ...
        try {
            ...
            if (urls != null && urls.size() > 0) {
                ...
                log.error("Failed to subscribe " + url + ", Using cached list: " + urls + " from cache file: " + getURL().getParameter(Constants.FILE_KEY, System.getProperty("user.home") + "/dubbo-registry-" + url.getHost() + ".cache") + ", cause: " + t.getMessage(), t);
            } else {
                // 如果开启了启动时检测，则直接抛出异常
                boolean check = getURL().getParameter(Constants.CHECK_KEY, true)
                        && url.getParameter(Constants.CHECK_KEY, true);
